...
//        in = new Scanner(new FileInputStream("./src/main/resources/codejam/year2020/round1b/A/A.in"));
    ...
    // we start at the end position and try to find the way backwards;
    // after every step we give, we halve the values (so that they always increase by steps of one instead of doubling)
    // with this invariant, at every step we should be able to have either (0,0) and found the end, or have an odd and an even number
    // since we go backwards, we also add the directions backwards (E instead of W if we go left for example)
